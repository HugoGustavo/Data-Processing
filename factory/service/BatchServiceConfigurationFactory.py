...
from configuration.service.BatchServiceConfiguration import BatchServiceConfiguration


class BatchServiceConfigurationFactory(object):
    __instance = None
    ...
    def build(self):
        ...
        server = DictionaryUtil.get(configuration, 'service.batch.server', '')
        server = ListUtil.get_as_list(server)
        server = ListUtil.map(lambda item: StringUtil.clean(item), server)

        self.__configuration = BatchServiceConfiguration(
            server=server
        )
