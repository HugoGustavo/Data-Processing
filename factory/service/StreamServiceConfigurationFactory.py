...
from configuration.service.StreamServiceConfiguration import StreamServiceConfiguration


class StreamServiceConfigurationFactory(object):
    __instance = None
    ...
    def build(self):
        ...
        server = DictionaryUtil.get(configuration, 'service.stream.server', '')
        server = ListUtil.get_as_list(server)
        server = ListUtil.map(lambda item: StringUtil.clean(item), server)

        self.__configuration = StreamServiceConfiguration(
            server=server
        )
